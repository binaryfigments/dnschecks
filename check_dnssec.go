...
// Get function
func checkDNSSEC(domain string, domainNS string, registryNS string) (*DNSSEC, error) {
	r := new(DNSSEC)
	...
	domainds, err := resolveDomainDS(domain, registryNS)
	if err != nil {
		return r, err
	}
	...
	dnskey, _ := resolveDomainDNSKEY(domain, domainNS)
	...
	if r.Answer.DSRecordCount > 0 && r.Answer.DNSKEYRecordCount > 0 {
		// Wel of geen error, geen probleem.
		calculatedDS, _ := calculateDSRecord(domain, digest, domainNS)
		r.Answer.CalculatedDS = calculatedDS
	}

	nsec, _ := resolveDomainNSEC(domain, domainNS)
	if nsec != nil {
		r.NSEC.Type = "nsec"
		r.NSEC.NSEC = nsec
	}

	nsec3, _ := resolveDomainNSEC3(domain, domainNS)
	if nsec3 != nil {
		r.NSEC.Type = "nsec3"
		r.NSEC.NSEC3 = nsec3
	}

	nsec3param, _ := resolveDomainNSEC3PARAM(domain, domainNS)
	if nsec3param != nil {
